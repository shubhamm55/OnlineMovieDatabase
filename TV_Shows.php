<html>
	...
	<body class="mystyle">
		...
		<div class="scrollable col-md-9">
				...
				$movies1 = $tmdb->getDiscoverTVShows(rand(1,1000));
				$movies2= $tmdb->getDiscoverTVShows(rand(1,1000));
				$movies3= $tmdb->getDiscoverTVShows(rand(1,1000));
				$movies4= $tmdb->getDiscoverTVShows(rand(1,1000));
				...
				foreach($res3 as $movie)
				{
					...
					<div class="hovereffect img-div">
						<img class="img-responsive list-img" src="'. $tmdb->getImageURL('w185') . $movie->getPoster() .'" alt="'. $movie->getName() .'">
							<div class="overlay">
           							<h2>'. $movie->getName() .'</h2>
           							<a class="info" href="TV_Landing.php?id='. $movie->getID() .'">Read More..</a>
        						</div>
        				</div>';
				}
				?>
